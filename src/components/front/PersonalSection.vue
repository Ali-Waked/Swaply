...
onMounted(async () => {
  try {
    const res = await axios.get("/api/user/reliability-score", {
      headers: {
        Authorization: `Bearer ${authStore.token}`, // أو حسب طريقة تخزينك للتوكن
      },
    });
    reliabilityScore.value = res.data.score;
  } catch (error) {
...
<template>
  ...
  >
    ...
    <div class="degree-ability mt-4" v-if="!loadingScore">
  <div class="flex items-center justify-between mb-2">
    <span class="font-[400] text-gray-700 dark:text-gray-300 text-[13px]">
      درجة الموثوقية
    </span>
  ...
  <div class="w-full h-2 rounded-md bg-blue-200 dark:bg-gray-700 relative">
    <div
      class="absolute left-0 top-0 h-full bg-blue-950 dark:bg-white rounded-md"
      :style="{ width: (reliabilityScore || 0) + '%' }"
    ></div>
  </div>
</div>
<p v-else class="text-sm text-gray-500 dark:text-gray-300">جارِ تحميل درجة الموثوقية...</p>
